...
var Ziwei = (function () {
  ...
  var _Ziwei_Models_ResultTable = function _Ziwei_Models_ResultTable() {
    var args = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];
    ...
    this.palaces = palaceConfigs.map(function (position, config) {
  ...
  _Ziwei_Calculator.prototype.calcResultTable = function () {
    ...
    branches.forEach(function (branch) {
      ...
      _this2.insertMultipleStarsToPalace(table[branch], stars, 'OtherImportantStars');

      stars = sixDeadlyStars[branch];
      _this2.insertMultipleStarsToPalace(table[branch], stars, 'SixDeadlyStars');

      stars = sixLuckyStars[branch];
      _this2.insertMultipleStarsToPalace(table[branch], stars, 'SixLuckyStars');

      var star = thaiTueConstellation[branch];
      _this2.insertSingleStarToPalace(table[branch], star, 'ThaiTueConstellation');

      star = locTonConstellation[branch];
      _this2.insertSingleStarToPalace(table[branch], star, 'LocTonConstellation');

      stars = normalStars[branch];
      _this2.insertMultipleStarsToPalace(table[branch], stars, 'NormalStars');

      stars = fourTransformationStars[branch];
      _this2.insertMultipleStarsToPalace(table[branch], stars, 'FourTransformationStars');
    });
